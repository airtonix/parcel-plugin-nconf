...
1. Fork [the repository](https://github.com/airtonix/parcel-plugin-nconf) and create your branch from `master`.
...
After cloning parcel-plugin-nconf, run `npm i` to fetch its dependencies.
Then, you can run several commands:

- `npm run dev` will build cjs and es module of parcel-plugin-nconf in the `lib` folder and watch for changes.
- `npm run lint` checks the code style.
- `npm test` runs the complete test suite.
- `npm test -- --watch` runs an interactive test watcher.
- `npm test <pattern>` runs tests with matching filenames.
- `npm run build` creates the cjs and es module of parcel-plugin-nconf in the `lib` folder.
